...
if (!function_exists('current_month_income')) {
    function current_month_income() {
        ...
        $monthly_income = \App\Models\Transaction::selectRaw("IFNULL(SUM(amount),0) as total")
            ->where("dr_cr", "cr")
            ->where("company_id", $company_id)
...
if (!function_exists('current_month_expense')) {
    function current_month_expense() {
        ...
        $monthly_expense = \App\Models\Transaction::selectRaw("IFNULL(SUM(amount),0) as total")
            ->where("dr_cr", "dr")
            ->where("company_id", $company_id)
            ->whereMonth("trans_date", $month)
            ->whereYear("trans_date", $year)
...
if (!function_exists('create_payment_method')) {
    function create_payment_method($methodName, $company_id) {
        $payment_method = \App\Models\PaymentMethod::where('name', $methodName)->where("company_id", $company_id);
...
if (!function_exists('increment_invoice_number')) {
    function increment_invoice_number() {
        ...
        if (\App\Models\CompanySetting::where('name', "invoice_starting")->where("company_id", $company_id)->exists()) {
            \App\Models\CompanySetting::where('name', 'invoice_starting')
                ->where("company_id", $company_id)
                ->update($data);
        } else {
            $data['name']       = 'invoice_starting';
            $data['created_at'] = date('Y-m-d H:i:s');
            \App\Models\CompanySetting::insert($data);
        }
    }
}
...
if (!function_exists('increment_quotation_number')) {
    function increment_quotation_number() {
        ...
        if (\App\Models\CompanySetting::where('name', "quotation_starting")->where("company_id", $company_id)->exists()) {
            \App\Models\CompanySetting::where('name', 'quotation_starting')
                ->where("company_id", $company_id)
                ->update($data);
        } else {
...
if (!function_exists('update_stock')) {
    function update_stock($product_id) {
        ...
        //Update Stock
        $stock = \App\Models\Stock::where("product_id", $product_id)->where("company_id", $company_id)->first();
        if ($stock) {

...
class MembershipController extends Controller {
    ...
    public function paystackPaymentAuthorize(Request $request, $payment_id)
    {
        $payment = PaymentHistory::find($payment_id);
        $increment = $payment->extend;
        ...
        try {
            ...
            // Process payment details and update your database as needed
            ...
            // Replace parameters for email template
            $replace = [
                '{name}' => $user->name,
                '{email}' => $user->email,
                '{valid_to}' => date(get_date_format(), strtotime($user->valid_to)),
            ...
            return redirect()->route('dashboard')->with('success', _lang('Thank you, You have successfully extended your membership.'));
        } catch (\Exception $e) {

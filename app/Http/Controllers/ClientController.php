...
class ClientController extends Controller {
    ...
    public function view_invoice($id) {
        ...
        return view('backend.client_panel.view_invoice', compact('invoice', 'invoice_taxes', 'transactions', 'company'));
    }
    ...
    public function download_pdf_invoice($id) {
        ...
        $data['company']      = CompanySetting::where('company_id', $invoice->company_id)->get();
    ...
    public function download_pdf_quotation($id) {
        ...
        $data['company'] = CompanySetting::where('company_id', $quotation->company_id)->get();
    ...
    public function view_quotation($id) {
        ...
        return view('backend.client_panel.view_quotation', compact('quotation', 'quotation_taxes', 'company'));
    }
    ...
    public function stripe_payment_authorize(Request $request, $invoice_id) {
        ...
        Stripe\Stripe::setApiKey(get_setting($company, 'stripe_secret_key'));
        $charge = Stripe\Charge::create([
            "amount"      => ($invoice->grand_total - $invoice->paid) * 100,
            "currency"    => get_setting($company, 'currency'),
        ...
        if ($charge->amount_refunded == 0 && $charge->failure_code == null && $charge->paid == true && $charge->captured == true) {
            ...
            $transaction->account_id        = get_setting($company, 'default_account');
            $transaction->chart_id          = get_setting($company, 'default_chart_id');
            $transaction->type              = 'income';
            $transaction->dr_cr             = 'cr';
            $transaction->amount            = $amount;
            $transaction->payer_payee_id    = $invoice->client_id;
            $transaction->payment_method_id = create_payment_method('Stripe', $invoice->company_id);
    ...
    public function paypal_payment_authorize(Request $request, $paypalOrderId, $invoice_id) {
        ...
        $clientId     = get_setting($company, 'paypal_client_id');
        $clientSecret = get_setting($company, 'paypal_secret');

        if (get_setting($company, 'paypal_mode') == 'sandbox') {
        ...
        try {
            ...
            if ($response->result->status == 'COMPLETED') {
                ...
                if ($amount >= ($invoice->grand_total - $invoice->paid)) {
                    ...
                    $transaction->amount            = $amount;
                    $transaction->payer_payee_id    = $invoice->client_id;
                    $transaction->payment_method_id = create_payment_method('PayPal', $invoice->company_id);
                    $transaction->invoice_id        = $invoice->id;
                    $transaction->company_id        = $invoice->company_id;
